...
// As of now it uses processing sound library but should be changed to Minim later

import processing.sound.*;

Amplitude rms;              // Define the root mean square of the amplitude 
AudioIn in;                 // Define the input
...
void setup() 
{
  ...
  in = new AudioIn(this, 0); // Create the Input stream
  in.play();
  
  rms = new Amplitude(this); // Create and patch the rms tracker
  rms.input(in);
}      
...
void draw() 
{
  ...
  sum += (rms.analyze() - sum) * smoothFactor;   // Smooth the rms data by smoothing factor - rms.analyze returns the current amplitude
